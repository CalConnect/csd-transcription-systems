...
authorities using identifiers, the list of which is maintained by ISO
{docnumber}/RA.
...
can be used, which must be placed at the beginning of the system code, and
followed by a "HYPHEN MINUS" character ("`-`", Unicode U+002D).
...
[example]
`ALA-LC:mal:Mlym:Latn:2012` (for the ALA-LC Romanization system that transliterates the Malayam language from Malayam script characters into Latin script. The system identifier here is considered to be `2012`) TODO : check example validity
